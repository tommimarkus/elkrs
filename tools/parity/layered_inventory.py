...
STATUS_OVERRIDES = {
    ...
    "org.eclipse.elk.layered.considerModelOrder.components": (
        ...
        "1.0.0 compatibility exclusion: component model-order crossing semantics are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.crossingCounterNodeInfluence": (
        ...
        "1.0.0 compatibility exclusion: crossing-counter node-order weighting is out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.crossingCounterPortInfluence": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked model_order_group`; `cargo test -p elkrs-layered --test basic_layout --locked model_order_group`; `cargo test -p elkrs-json --test json_partitions --locked model_order_group`; `cargo test -p elkrs-json --test json_errors --locked model_order_group`",
        "1.0.0 compatibility exclusion: crossing-counter port-order weighting is out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.groupModelOrder.cbGroupOrderStrategy": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked model_order_group`; `cargo test -p elkrs-layered --test basic_layout --locked model_order_group`; `cargo test -p elkrs-json --test json_partitions --locked model_order_group`; `cargo test -p elkrs-json --test json_errors --locked model_order_group`",
        "1.0.0 compatibility exclusion: cycle-breaking group-order semantics are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.groupModelOrder.cbPreferredSourceId": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked model_order_group`; `cargo test -p elkrs-layered --test basic_layout --locked model_order_group`; `cargo test -p elkrs-json --test json_partitions --locked model_order_group`; `cargo test -p elkrs-json --test json_errors --locked model_order_group`",
        "1.0.0 compatibility exclusion: cycle-breaking preferred-source group-order semantics are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.groupModelOrder.cbPreferredTargetId": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked model_order_group`; `cargo test -p elkrs-layered --test basic_layout --locked model_order_group`; `cargo test -p elkrs-json --test json_partitions --locked model_order_group`; `cargo test -p elkrs-json --test json_errors --locked model_order_group`",
        "1.0.0 compatibility exclusion: cycle-breaking preferred-target group-order semantics are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.groupModelOrder.cmEnforcedGroupOrders": (
        ...
        "1.0.0 compatibility exclusion: crossing-minimization enforced group orders are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.groupModelOrder.cmGroupOrderStrategy": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked model_order_group`; `cargo test -p elkrs-layered --test basic_layout --locked model_order_group`; `cargo test -p elkrs-json --test json_partitions --locked model_order_group`; `cargo test -p elkrs-json --test json_errors --locked model_order_group`",
        "1.0.0 compatibility exclusion: crossing-minimization group-order strategy semantics are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.groupModelOrder.componentGroupId": (
        ...
        "1.0.0 compatibility exclusion: component group-order semantics plus edge and port targets are out of scope; node-scope IDs are parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.groupModelOrder.crossingMinimizationId": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked model_order_group_id`; `cargo test -p elkrs-layered --test basic_layout --locked model_order_group_ids`; `cargo test -p elkrs-json --test json_partitions --locked model_order_group_ids`; `cargo test -p elkrs-json --test json_errors --locked model_order_group_id`",
        "1.0.0 compatibility exclusion: crossing-minimization group IDs plus edge and port targets are out of scope; node-scope IDs are parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.groupModelOrder.cycleBreakingId": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked model_order_group_id`; `cargo test -p elkrs-layered --test basic_layout --locked model_order_group_ids`; `cargo test -p elkrs-json --test json_partitions --locked model_order_group_ids`; `cargo test -p elkrs-json --test json_errors --locked model_order_group_id`",
        "1.0.0 compatibility exclusion: cycle-breaking group-order semantics are out of scope; node-scope IDs are parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.longEdgeStrategy": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked model_order_group`; `cargo test -p elkrs-layered --test basic_layout --locked model_order_group`; `cargo test -p elkrs-json --test json_partitions --locked model_order_group`; `cargo test -p elkrs-json --test json_errors --locked model_order_group`",
        "1.0.0 compatibility exclusion: long-edge model-order semantics are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.portModelOrder": (
        "diagnostic",
        "`cargo test -p elkrs-layered --test basic_layout --locked parent_boolean`; `cargo test -p elkrs-json --test json_partitions --locked parent_boolean`; `cargo test -p elkrs-json --test json_errors --locked parent_boolean`",
        "1.0.0 compatibility exclusion: port-order-aware crossing behavior is out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.strategy": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked model_order`; `cargo test -p elkrs-layered --test basic_layout --locked model_order`; `cargo test -p elkrs-json --test json_partitions --locked model_order`; `cargo test -p elkrs-json --test json_errors --locked model_order`",
        "1.0.0 compatibility exclusion: model-order crossing strategy semantics are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.considerModelOrder.noModelOrder": (
        "diagnostic",
        "`cargo test -p elkrs-layered --test basic_layout --locked node_boolean`; `cargo test -p elkrs-json --test json_partitions --locked node_boolean`; `cargo test -p elkrs-json --test json_errors --locked node_boolean`",
        "1.0.0 compatibility exclusion: no-model-order crossing constraints are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.crossingMinimization.forceNodeModelOrder": (
        "diagnostic",
        "`cargo test -p elkrs-layered --test basic_layout --locked parent_boolean`; `cargo test -p elkrs-json --test json_partitions --locked parent_boolean`; `cargo test -p elkrs-json --test json_errors --locked parent_boolean`",
        "1.0.0 compatibility exclusion: forced node model-order crossing constraints are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.crossingMinimization.greedySwitch.activationThreshold": (
        ...
        "1.0.0 compatibility exclusion: greedy-switch crossing minimization is out of scope; the threshold is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.crossingMinimization.greedySwitch.type": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked greedy_switch`; `cargo test -p elkrs-layered --test basic_layout --locked greedy_switch`; `cargo test -p elkrs-json --test json_partitions --locked greedy_switch`; `cargo test -p elkrs-json --test json_errors --locked greedy_switch`",
        "1.0.0 compatibility exclusion: greedy-switch crossing minimization is out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.crossingMinimization.greedySwitchHierarchical.type": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked greedy_switch`; `cargo test -p elkrs-layered --test basic_layout --locked greedy_switch`; `cargo test -p elkrs-json --test json_partitions --locked greedy_switch`; `cargo test -p elkrs-json --test json_errors --locked greedy_switch`",
        "1.0.0 compatibility exclusion: hierarchical greedy-switch crossing minimization is out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.crossingMinimization.hierarchicalSweepiness": (
        ...
        "1.0.0 compatibility exclusion: hierarchical crossing sweepiness semantics are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.crossingMinimization.inLayerPredOf": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked crossing_minimization_controls`; `cargo test -p elkrs-layered --test basic_layout --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_partitions --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_errors --locked crossing_minimization_controls`",
        "1.0.0 compatibility exclusion: in-layer predecessor ordering constraints are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.crossingMinimization.inLayerSuccOf": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked crossing_minimization_controls`; `cargo test -p elkrs-layered --test basic_layout --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_partitions --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_errors --locked crossing_minimization_controls`",
        "1.0.0 compatibility exclusion: in-layer successor ordering constraints are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.crossingMinimization.positionChoiceConstraint": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked crossing_minimization_controls`; `cargo test -p elkrs-layered --test basic_layout --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_partitions --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_errors --locked crossing_minimization_controls`",
        "1.0.0 compatibility exclusion: position-choice-constrained crossing minimization is out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.layered.crossingMinimization.positionId": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked crossing_minimization_controls`; `cargo test -p elkrs-layered --test basic_layout --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_partitions --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_errors --locked crossing_minimization_controls`",
        "1.0.0 compatibility exclusion: input position-ID-constrained crossing minimization is out of scope; generated position IDs are Java-backed under `LAYERED-META-OPTION-057`",
    ),
    "org.eclipse.elk.layered.crossingMinimization.strategy": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked crossing_minimization_controls`; `cargo test -p elkrs-layered --test basic_layout --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_partitions --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_errors --locked crossing_minimization_controls`",
        "1.0.0 compatibility exclusion: alternate crossing-minimization strategies are out of scope; current barycenter-style ordering remains Java-backed under `LAYERED-P3-001`",
    ),
    "org.eclipse.elk.layered.crossingMinimization.semiInteractive": (
        "diagnostic",
        "`cargo test -p elkrs-layered --test basic_layout --locked parent_boolean`; `cargo test -p elkrs-json --test json_partitions --locked parent_boolean`; `cargo test -p elkrs-json --test json_errors --locked parent_boolean`",
        "1.0.0 compatibility exclusion: semi-interactive position-derived crossing minimization is out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    ...
    "org.eclipse.elk.layered.thoroughness": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked crossing_minimization_controls`; `cargo test -p elkrs-layered --test basic_layout --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_partitions --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_errors --locked crossing_minimization_controls`",
        "1.0.0 compatibility exclusion: crossing-minimization thoroughness and sweep iteration tuning are out of scope; the option is parsed, serialized, and diagnosed only",
    ),
    "org.eclipse.elk.randomSeed": (
        "diagnostic",
        "`cargo test -p elkrs-core --locked crossing_minimization_controls`; `cargo test -p elkrs-layered --test basic_layout --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_partitions --locked crossing_minimization_controls`; `cargo test -p elkrs-json --test json_errors --locked crossing_minimization_controls`",
        "1.0.0 compatibility exclusion: random-dependent crossing-minimization behavior is out of scope; the option is parsed, serialized, and diagnosed only",
    ),
